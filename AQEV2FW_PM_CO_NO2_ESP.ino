...
#define EEPROM_PM1P0_CAL_OFFSET   (EEPROM_CO_ZERO_NEGATIVE_RESULTS - 4)            // offset value for PM1P0 in ug/m^3
...
// backup status bits
#define BACKUP_STATUS_PARTICULATE_CALIBRATION_BIT (8)
...
void initializeHardware(void) {
    ...
    Serial.println(F(" |     NO2 / CO / PM Sensor Suite     |"));
...
void print_eeprom_value(char * arg) {
    if (strncmp(arg, "mac", 3) == 0) {
        ...
        print_eeprom_float((const float *) EEPROM_NO2_CAL_OFFSET);
        Serial.print(F("    "));
        Serial.print(F("NO2 Report Negative values as zero: "));
        ...
        Serial.print(F("    "));
        Serial.println(F("NO2 Baseline Voltage Characterization:"));
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        eeprom_write_block("aqev2_pm_co_no2_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"particulate\",\"co\",\"no2\",\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
